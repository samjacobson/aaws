...
class SNS(AWSService):
	...
	def CreatePlatformEndpoint(self, PlatformApplicationArn, Token, CustomUserData=None, Attributes=None):
		...
		def response(status, reason, data):
			if status == 200:
				root = ET.fromstring(data)
				node = root.find('.//{http://sns.amazonaws.com/doc/2010-03-31/}EndpointArn')
				if node is not None:
					return node.text
			raise AWSError(status, reason, data)

		r = request.AWSRequest(self._endpoint, '/', self._key, self._secret, 'CreatePlatformEndpoint', {
	...
	def Publish(self, TargetArn, Message, Subject=None, MessageStructure=None):
		"""The Publish action sends a message to all of a topic's subscribed endpoints. When a
			...
			TargetArn -- The topic or endpoint you want to publish to.
				Type: String
				Required: Yes
